...
You can run this project in a containerized environment using Docker Compose. This setup ensures all dependencies and the correct Node.js version are used automatically.

### Project-specific Docker Details

- **Node.js version:** The Docker image uses Node.js v22.13.1 (slim variant) as specified in the Dockerfile.
- **Dependencies:** All production dependencies are installed via `npm ci --production` during the build.
- **User permissions:** The app runs as a non-root user for improved security.
- **Environment variables:**
  - `NODE_ENV=production` (set by default)
  - `NODE_OPTIONS=--max-old-space-size=4096` (set by default)
- **Ports:** No ports are exposed, as the application is a CLI tool and does not run a persistent server.

### Build and Run Instructions

1. **Ensure Docker Desktop is running and Docker Model Runner is enabled as described above.**
...
This will build the Docker image and run the application in a container named `js-app`.

- No ports are exposed, and no volumes or additional services are required.
- If you need to pass environment variables, you can uncomment and use the `env_file` section in `docker-compose.yml`.

### Special Notes

- The application will execute its workflow and then exit. You can view the output in the container logs.
- All results will be saved inside the container's filesystem unless you mount a volume for the `data/` directory.
- If you need to re-run the workflow, simply re-run the `docker compose up --build` command.

For advanced configuration, refer to the `docker-compose.yml` and `Dockerfile` for further customization options.
